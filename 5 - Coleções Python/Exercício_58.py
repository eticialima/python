"""
58) Faça um programa que leia uma matriz de 5 linhas e 4 colunas contendo
as seguintes informações sobre alunos.txt de uma disciplina, sendo todas as informações do
tipo inteiro:
...
while len(alunos) < 5:
    ...
    while len(lista2) < 3:
        if cont2 == 0:
            matricula = int(input(f"Enter student enrollment number {cont1 + 1}: "))
            ...
            # até informar uma que não foi digitada
                ...
                print("Registration already typed\n")
                cont2 -= 1
                pass

        elif cont2 == 1:
            media_prova = int(input(f"Enter the average of the student's exams {cont1 + 1}: "))
            lista2.append(media_prova)

        elif cont2 == 2:
            media_trabalho = int(input(f"Enter the average student's work {cont1 + 1}: "))
...
print(f"\nRegistration of the student who obtained the highest final grade: {matricula}")
print(f"The arithmetic mean of the final grades: {media_aritmetica}")
